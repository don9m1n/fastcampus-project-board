...
@DisplayName("[Service] 비즈니스 로직 - 게시글")
@ExtendWith(MockitoExtension.class)
class ArticleServiceTest {

    @InjectMocks // 실제 인스턴스를 생성하고 @Mock, @Spy 가 붙은 가짜 객체를 주입 받음 (Mock 객체를 주입하는 대상)
    ...
    void givenArticleId_whenDeletingArticle_thenDeletesArticle() {
        // Given
        Long articleId = 1L;
        String userId = "DongminTest";
        willDoNothing().given(articleRepository).deleteArticleByIdAndUserAccount_UserId(articleId, userId);

        // When
        sut.deleteArticle(articleId, userId);

        // Then
        then(articleRepository).should().deleteArticleByIdAndUserAccount_UserId(articleId, userId);
    }
